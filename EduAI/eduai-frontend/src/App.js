...
const App = () => {
  ...
  const [isSubmitted, setIsSubmitted] = useState(false); // State to track if HomePage is submitted
  ...
  const handleAnalyzeClick = async (youtubeUrl) => {
    if (youtubeUrl) {
      if (isValidYouTubeUrl(youtubeUrl)) {
        try {
          ...
          setVideoUrl(youtubeUrl); // Update the video URL
          setInputUrl(''); // Clear the input field
          setError(''); // Clear any existing error
          setIsSubmitted(true); // Set to true to switch views
        } catch (error) {
  ...
  return (
    <Router>
      {/* Conditionally render HomePage or the main app */}
      {!isSubmitted ? (
        <HomePage onSubmit={handleAnalyzeClick} /> // Pass the function here
      ) : (
